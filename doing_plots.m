...
v_sel         = {'p_N', 'tby'}; %nombre de las variables cuyo irf desean mostrarse, con el mismo nombre que el declarado en Dynare
v_name        = {'1/rer', 'tby_t'};%nombre que aparecera en el titulo de cada IRF para cada variable ex�gena. Se permite escribir en formato LaTex.
...
plot_options.horizon    = 100; % numero que indica el horizonte al mostrar de la IRF, que debe ser menor o igual al numero en la opcion irf de stcoh_simul en Dynare.
plot_options.n_col      = 3; %numero de columnas en el sub-plot.
plot_options.n_row      = 1; %numero de filas en el sub-plot. El  numero de filas multiplicado por el de columnas debe ser mayor o igual al numero de variables que quieren ser graficadas +1 (recordando que el programa siempre grafica la respuesta de la variable que esta siendo shockeada).
plot_options.grid       = 0; % 1 si se incluye una grilla de puntos en el grafico
plot_options.latex      = 1; % 1 si los titulos son escritos con formato Latex
plot_options.marks={'-blue','--r','-.blue',':k'}; % Tipo de linea para cada modelo, segun el orden en "model_names" (solida azul, punteada roja, puntos y rayas negra, puntos magenta)
